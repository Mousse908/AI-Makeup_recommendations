...
result = ['Image', "Eye Ball Distance", "Eye Distance", "Eye Width", "Face Width",
    ...
    "Face Length", "Face area", "Area Rate","Jaw Length", "Forehead Length"]
...
def calculate_face(landmarks):
    ...
    face_width = math.sqrt((right_face.x - left_face.x) ** 2 + (right_face.y - left_face.y) ** 2) #right_face.x(該index點的x座標)
...
for image_name in os.listdir(image_folder):
    if image_name.endswith('.jpg'):
        ...
        if results.multi_face_landmarks:
            ...
            jaw_left = landmarks[JAW_INDEX[0]]
            jaw_right = landmarks[JAW_INDEX[1]]
            forehead_left = landmarks[FOREHEAD_INDEX[0]]
            forehead_right = landmarks[FOREHEAD_INDEX[1]]
            ...
            jaw_length = get_distance(jaw_left,jaw_right)
            forehead_length = get_distance(forehead_left,forehead_right)
            ...
            jaw_length = Coordinate_normalization(jaw_length, standard_value)
            forehead_length = Coordinate_normalization(forehead_length, standard_value)
...
# 將特徵值寫入CSV文件
output_csv = 'csv_file/face_data_area.csv'
